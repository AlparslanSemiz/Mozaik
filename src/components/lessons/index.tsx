...
export default function Lessons({
  ...
  // The ribbon already fixes one axis in the focused modes. If that same axis
  // remains in the facet row it offers a single chip that cannot narrow
  // anything. Drop its old value too, otherwise `isFiltering()` would keep the
  // list locked even though the filter that caused it is no longer visible.
  useEffect(() => {
    const hiddenFacet =
      mode === "teacher" ? "ogretmen" : mode === "class" ? "sinif" : null;
    if (hiddenFacet === null) return;
    setQuery((current) => {
      if (!(hiddenFacet in current.facets)) return current;
      const facets = { ...current.facets };
      delete facets[hiddenFacet];
  ...
  const scope =
    mode === "all" || focused === undefined
      ...
      : state.lessons.filter((x) =>
          mode === "teacher"
            ? x.teacherId === focused.id
            : x.classId === focused.id,
        );
  ...
  const teacherId =
    mode === "teacher" ? (focused?.id ?? "") : newLesson.teacherId;
  ...
  const teacherSubs =
    newTeacherObj === undefined ? [] : teacherSubjects(newTeacherObj);
  const subjectPool =
    mode === "teacher" && newTeacherObj !== undefined
      ? teacherSubs
  ...
  const teacherPool =
    newLesson.subject === ""
      ? state.teachers
      : subjectTeachers(state, newLesson.subject);
  ...
  function chooseSubject(name: string) {
    ...
    const drops =
      name !== "" &&
      newTeacherObj !== undefined &&
      slotOf(teacherSubs, name) < 0 &&
      mode !== "teacher";
    setNewLesson({
      ...newLesson,
      subject: name,
  ...
  const counted = t("{ders} ders · {saat} saat", {
    ders: scope.length,
    saat: scopeHours,
  });
  const heading =
    ...
    <>
        ...
        >
          {mode !== "class" && (
            <select
              ...
              onChange={(e) =>
                setNewLesson({ ...newLesson, classId: e.target.value })
              }
            >
          ...
          <Field label={t("Haftalık saat")}>
            <input
              ...
              onChange={(e) =>
                setNewLesson({ ...newLesson, hours: e.target.value })
              }
            />
          </Field>
      ...
      <div className="panel step-panel">
        ...
        {scope.length === 0 && state.lessons.length > 0 && mode !== "all" && (
          <p className="hint">
            Bu {modeNoun} için henüz ders girilmemiş. Yukarıdaki satırdan
            ekleyin.
          </p>
        )}
                    ...
                    <tr key={x.id} data-row-name={rowName}>
                      ...
                      <td>
                        <span
                          className="color-dot"
                          style={{
                            background: paletteColor(group?.color ?? 0),
                          }}
                        />{" "}
                        {group?.name ?? "?"}
                      </td>
                      <td>
                        <span
                          className="color-dot"
                          style={{
                            background: paletteColor(teacher?.color ?? 0),
                          }}
                        />{" "}
                        {teacher?.short ?? "?"}{" "}
                      ...
                      <td>
                        <BlockCounts
                          ...
                          onPick={(blocks) =>
                            change((d) => updateLesson(d, x.id, { blocks }))
                          }
                        />
                      </td>
                      <td>
                        <LimitBox
                          value={x.maxPerDay}
                          fallback={
                            group?.maxSameLessonPerDay ??
                            state.settings.limits.maxSameLessonPerDay
                          }
                          title={t("Bu ders bir günde en fazla kaç saat")}
                          onSet={(v) =>
                            change((d) =>
                              updateLesson(d, x.id, { maxPerDay: v }),
                            )
                          }
                        />
                      </td>
    ...
    <div className="cols">
      ...
      <aside>
        {mode === "all" ? (
          ...
          <div className="panel">
            <h2>
              {mode === "teacher" ? t("Hangi öğretmen") : t("Hangi sınıf")}
            </h2>
