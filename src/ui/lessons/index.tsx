...
export default function Lessons({ state, change, mode, focus, setFocus }: Props) {
  ...
  useEffect(() => {
    const hiddenFacet = mode === 'teacher' ? 'ogretmen' : mode === 'class' ? 'sinif' : null;
    ...
    <>
      ...
      >
        ...
        <Paste
          ...
          example={t('Sınıf · Öğretmen (ad veya kısaltma) · Haftalık saat · Blok (1 · 2 · 3)')}
          parse={parseLessons}
          rowText={(x) =>
      ...
      <aside>
        {mode === 'all' ? (
          ...
          <div className="panel">
                ...
                return (
                  ...
                  >
                    ...
                    <span className="entity-count">
                      {count} ders ·{' '}
                      {weeklyLoad(state, mode === 'teacher' ? 'teacher' : 'class', x.id)} saat
                    </span>
                  </button>
                );
              })}
            ...
            {picking.length === 0 && (
              <p className="hint">
                <T k="Henüz {ne} yok. **Okul** sekmesinden ekleyin." vars={{ ne: t(modeNoun) }} />
              </p>
            )}
          </div>
        )}
