...
describe('yük durumu sınırları', () => {
  it.each([
    ...
    [100, 'tight'],
    [101, 'impossible'],
  ] as const)('100 saat kapasitede %s saat yükü %s sınıfına girer', (load, level) => {
    expect(loadStatus(100, load)).toBe(level);
  });

  it('rapor seviyesi boş yükü ayrı bir sorun seviyesi yapmaz', () => {
