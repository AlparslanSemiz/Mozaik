...
- **`.pool-card` bekleyen bir blok demek.** Havuz testlerinin çoğu onu sayıyor, "N blok bekliyor" oradan geliyor, ve `pendingBlocks()`'un aynası o. Deste bir düzen, bir gruplama değil: aynı sayıda öğe DOM'da kalır, gömülü olanlar yalnız mürekkebini bırakır. `visibility` değil, çünkü gizli bir öğenin `innerText`'i boş ve `allInnerTexts()` okuyan testler tepsiyi dağılmış sanar. Yan faydası: haftalık saati elle aşmak mümkün değil, kart bitince sürüklenecek bir şey kalmıyor.
