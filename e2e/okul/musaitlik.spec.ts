...
test.describe("10. Müsaitlik çizelgesi", () => {
  test("üst çizelgeyle haftanın darlığı aynı satır yüksekliğini kullanıyor", async ({
    ...
    const upper = page
      ...
      .first();
    const heat = page.locator("table.availability.heat tbody tr").first();
    await expect(upper).toBeVisible();
    await expect(heat).toBeVisible();
    const [upperBox, heatBox] = await Promise.all([
      upper.boundingBox(),
      heat.boundingBox(),
    ]);
    expect(Math.abs(upperBox!.height - heatBox!.height)).toBeLessThanOrEqual(1);
  });

  test("satır = gün, sütun = ders; gün satırına tıklayınca o gün kapanıyor", async ({
    page,
  }) => {
    await openWithSample(page);
    await page.getByRole("button", { name: "Müsaitlik" }).click();
    ...
    await expect(
      table.locator("tbody tr").first().locator("td.closed"),
    ).toHaveCount(0);
  ...
  test("öğle arası her satırda kendi yerinde işaretli", async ({ page }) => {
    ...
    const positions = await page.evaluate(() =>
      [
        ...document.querySelectorAll("table.availability:not(.heat) tbody tr"),
      ].map((row) =>
        [...row.querySelectorAll("td")].findIndex((td) =>
          td.classList.contains("break-after"),
        ),
      ),
    );
...
test.describe("20. Kapalı saatte ders", () => {
  ...
  test("Kontrol sekmesi tek tek sayıyor ve sebebini yazıyor", async ({
    page,
  }) => {
    await conflict(page);
    ...
    await expect(
      panel.getByRole("heading", { name: /Kapalı saatte ders/ }),
    ).toBeVisible();
  ...
  test("Müsaitlik ekranı olan biteni söylüyor", async ({ page }) => {
    await conflict(page);
    await expect(
      page.locator(".warn-box", { hasText: "yerleşmiş" }),
    ).toContainText("Hiçbiri silinmedi");
  });
  ...
  test("saat yeniden açılınca işaret kalkıyor", async ({ page }) => {
    ...
    await page.getByRole("button", { name: "Program", exact: true }).click();
    await expect(
      page.locator("table.grid .card.conflict").first(),
    ).toBeVisible();
...
test.describe("67. Kapalı saatin işareti", () => {
  test("çarpı KIRMIZI, büyük, ve kendi zemininde okunuyor", async ({
    page,
  }) => {
    await openWithSample(page);
    await page.getByRole("button", { name: "Müsaitlik" }).click();
    ...
    expect(
      seen.hatched,
      "tarama kalkmış — renk tek başına durum taşıyamaz",
    ).toBe(true);
    expect(
      seen.size,
      `çarpı ${seen.size}px, gövde ${seen.body}px`,
    ).toBeGreaterThan(seen.body);
    ...
    expect(
      ratio,
      `çarpı/zemin kontrastı ${ratio.toFixed(2)}`,
